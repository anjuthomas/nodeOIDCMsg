...
var ms = require('ms');
var ScopedAccessToken = require('../node_modules/tokenProfiles/scopedAccessToken');
  ...
  Object.keys(algorithms).forEach(function (algorithm) {
    ...
    describe('when signing a token with standard claim', function () {
      ...
      it('should throw when invalid standard claim', function (done) {
        try{
          var decodedPayload = scopedAccessToken.fromJWT(signedJWT, 'shhhh', {"iss" : "wrong-issuer", "sub": "subject", "aud" : "audience", 'maxAge': '1d', 'clockTolerance' : 10, "scope": "scope"}, {'clockTimestamp' : clockTimestamp});
        }catch(err){
