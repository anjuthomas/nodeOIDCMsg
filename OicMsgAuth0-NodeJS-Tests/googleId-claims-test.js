  ...
  Object.keys(algorithms).forEach(function (algorithm) {
    describe(algorithm, function () {
      ...
      describe('when signing a token with a known non standard claim', function () {
        ...
        it('should check known non standard claim', function (done) {
          try{
            var decodedPayload = googleIdToken.fromJWT(signedJWT, 'shhhh', {"name" : "name", "email": "email@google.com", "picture":  '/pathToPic', "iss": "issuer", "aud" : "audience", 'maxAge': '1d', 'clockTolerance' : 10, "sub": "subject"},{'clockTimestamp' : clockTimestamp});
        ...
        it('should throw when invalid known non standard claim', function (done) {
          ...
          try{
            var decodedPayload = googleIdToken.fromJWT(signedJWT, 'shhhh', {"name" : "name", "email": "email@google.com", "picture":  '/pathToPic', "iss": "issuer", "aud" : "wrong-audience", 'maxAge': '1d', 'clockTolerance' : 10, "sub": "subject"}, {'clockTimestamp' : clockTimestamp});
      ...
      describe('when signing a token without standard claim', function () {
        it('should throw error and require standard claim', function (done) {
          try{
            var googleIdToken = new GoogleIdToken('name', 'email@google.com', '/pathToPic', 'issuer','subject');
            googleIdToken.addNonStandardClaims({"aud" : "audience", "exp" : clockTimestamp + 3});
            googleIdToken.setNoneAlgorithm(true);
            var signedJWT = googleIdToken.toJWT('shhhh');
          }catch(err){
    ...
    describe('when signing a token with standard claim', function () {
      ...
      it('should throw when invalid standard claim', function (done) {
        try{
          var decodedPayload = googleIdToken.fromJWT(signedJWT, 'shhhh', {"name":"name", "email": "email@google.com", "picture":"/pathToPic", "iss" : "wrong-issuer", "sub": "subject", 'maxAge': '1d'}, {'clockTimestamp' : clockTimestamp});
        }catch(err){
