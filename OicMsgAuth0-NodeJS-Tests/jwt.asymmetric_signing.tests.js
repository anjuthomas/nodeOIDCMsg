    ...
    describe(algorithm, function () {
      ...
      describe('when signing a token', function () {
        ...
        context('asynchronous', function () {
          it('should validate with public key', function (done) {
            ...
            var decoded = basicIdToken.fromJWT(token, pub, {"foo": "bar", "iss" : "issuer", "sub": "subject", "aud" : "audience", 'maxAge': '3s', 'clockTolerance' : 10, "jti": "jti"}, {"clockTimestamp" : clockTimestamp});
          ...
          it('should throw with invalid public key', function (done) {
            try{
                var decoded = basicIdToken.fromJWT(token, invalid_pub, {"foo": "bar", "iss" : "issuer", "sub": "subject", "aud" : "audience", 'maxAge': '3s', 'clockTolerance' : 10, "jti": "jti"}, {"clockTimestamp" : clockTimestamp});
            }catch(err){
        ...
        context('synchronous', function () {
          ...
          it('should throw with invalid public key synchronous', function (done) {
            try{
                var decoded = basicIdToken.fromJWT(token, invalid_pub, {"foo": "bar", "iss" : "issuer", "sub": "subject", "aud" : "audience", 'maxAge': '3s', 'clockTolerance' : 10, "jti": "jti"}, {"clockTimestamp" : clockTimestamp});
            }catch(err){
                assert.isUndefined(decoded);
      ...
      describe('when signing a token with expiration', function () {
        ...
        it('should NOT be invalid', function (done) {
          ...
          var decoded = basicIdToken.fromJWT(token, pub, {"foo": "bar", "iss" : "issuer", "sub": "subject", "aud" : "audience", 'maxAge': '3s', 'clockTolerance' : 10, "jti": "jti"}, {"clockTimestamp" : clockTimestamp, ignoreExpiration: true });
          assert.ok(decoded.iss);
          assert.equal('issuer', decoded.iss);
          done();
        });
      ...
      describe('when signing a token with not before', function () {
        ...
        it('should be valid expiration', function (done) {
          ...
          try{
            var decoded = basicIdToken.fromJWT(token, "shhh", {"foo": "bar", "iss" : "issuer", "sub": "subject", "aud" : "audience", 'maxAge': '3s', 'clockTolerance' : 500, "jti": "jti"}, {"clockTimestamp" : clockTimestamp});
          }catch(err){
            assert.isNotNull(decoded);
            assert.isNull(err);
          } 
        ...
        it('should NOT be invalid', function (done) {
          ...
          jwt.verify(token, pub, { ignoreNotBefore: true }, function (err, decoded) {
            ...
            var decoded = basicIdToken.fromJWT(token, pub, {"foo": "bar", "iss" : "issuer", "sub": "subject", "aud" : "audience", 'maxAge': '3s', 'clockTolerance' : 10, "jti": "jti"}, {"clockTimestamp" : clockTimestamp, ignoreNotBefore: true});
            assert.ok(decoded.iss);
            assert.equal('issuer', decoded.iss);
            done();
        });
      ...
      describe('when signing a token with jwt id', function () {
        ...
        it('should throw when invalid jwt id', function (done) {
            try{
                var decoded = basicIdToken.fromJWT(token, pub, {"foo": "bar", "aud" : "audience", "iss" : "issuer", "sub": "subject", 'maxAge': '3s', 'clockTolerance' : 10, "jti": "jti"}, {"clockTimestamp" : clockTimestamp, jwtid: 'wrongJwtid' });
              }catch(err){
                assert.isUndefined(decoded);
                assert.isNotNull(err);
                assert.equal(err.name, 'JsonWebTokenError');
