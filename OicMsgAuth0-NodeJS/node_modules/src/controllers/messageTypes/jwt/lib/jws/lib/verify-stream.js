...
function isValidJws(string, baseEncoding) {
  //return JWS_REGEX.test(string) && !!headerFromJWSBase32(string);
  ...
  switch(baseEncoding) {
    ...
    case "base32":
      ...
      break;
    default:
      return JWS_REGEX.test(string) && !!headerFromJWSBase(string);
  }
...
/*
function jwsDecode(jwtString, secretOrPublicKey, tokenProfile, options, callback) {
...
function jwsDecodeBase16(jwtString, secretOrPublicKey, tokenProfile, options, callback) {
  ...
  var baseEncoding = options.baseEncoding;
  ...
  switch(baseEncoding) {
    ...
    case "base32":
      ...
      break;
    default:
      header = headerFromJWSBase(jwtString);  
      payload = payloadFromJWSBase(jwtString);      
    }
  //var header = headerFromJWSBase32(jwtString);

  if (!header)
    return null;

  //var payload = payloadFromJWSBase32(jwtString);
  if (header.typ === 'JWT' || options.json)
    payload = JSON.parse(payload, options.encoding);
